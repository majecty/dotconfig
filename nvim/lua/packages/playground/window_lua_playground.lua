--- Window Lua Playground: Write Lua code to learn windows as independent viewports
--- Interactive environment with window-focused API examples and templates
...
local function show_help()
  ...
  local help = {
    '',
    '╔══════════════════════════════════════════════════════════════╗',
    '║        WINDOW LUA PLAYGROUND - Learn Windows                ║',
    '╚══════════════════════════════════════════════════════════════╝',
    '',
    '📝 Write Lua code to explore windows! Press <C-x> to execute.',
    '',
    '📚 WINDOW CONCEPTS:',
    '',
    '  Windows are viewports into buffers',
    '  Each window shows ONE buffer at a time',
    '  Multiple windows can show same buffer',
    '  Windows have independent cursors and views',
    '  Windows are created by splits',
    '',
    '🔍 KEY FUNCTIONS:',
    '',
    '  vim.api.nvim_list_wins()              - All window IDs',
    '  vim.api.nvim_get_current_win()        - Current window ID',
    '  vim.api.nvim_set_current_win(win)     - Focus window',
    '  vim.api.nvim_win_get_buf(win)         - Buffer shown in win',
    '  vim.api.nvim_win_set_buf(win, buf)    - Show buffer in win',
    '  vim.api.nvim_win_get_height(win)      - Height in lines',
    '  vim.api.nvim_win_set_height(win, h)   - Set height',
    '  vim.api.nvim_win_get_width(win)       - Width in cols',
    '  vim.api.nvim_win_set_width(win, w)    - Set width',
    '  vim.api.nvim_win_get_position(win)    - (row, col) position',
    '  vim.api.nvim_win_get_cursor(win)      - Cursor (row, col)',
    '  vim.api.nvim_win_set_cursor(win, pos) - Set cursor',
    '',
    '💡 EXAMPLE PATTERNS:',
    '',
    '  -- Inspect all windows:',
    ...
    '    print(string.format("Win %d: buf %d, %dx%d", i, buf, w, h))',
    '  end',
    '',
    '  -- Switch windows:',
    '  local wins = vim.api.nvim_list_wins()',
    ...
    '🎯 TRY THIS:',
    '',
    '  1. Create a split first (use vim.cmd)',
    '  2. List all windows',
    '  3. Get their sizes and buffers',
...
function M.start()
  ...
  set_buffer_content(code_buf, {
    '-- Create a split and inspect windows',
    'vim.cmd("split")',
    '',
    'local wins = vim.api.nvim_list_wins()',
    ...
    '  print(string.format("Win %d%s: buf %d, pos(%d,%d), size %dx%d",',
    '    i, is_current and " [CURRENT]" or "", buf, row, col, w, h))',
    'end',
  })
  ...
  print('🪟 Window Lua Playground started! Press Ctrl+x to execute code')
